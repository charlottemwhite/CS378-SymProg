;   db.clj        Tiny Database Program       ; 20 Nov 20
;   27 Nov 18; 26 Apr 19; 06 Dec 19
;   Gordon Novak
...
(defn ask [sentence]
  ...
  (initsent sentence)
  (s)
  (if (or (empty? restrictions) (empty? retrievals) (not (empty? atnsent)))
      (do (if (not (empty? atnsent))
              (println "words not parsed:" failurept))
          (if (empty? restrictions)
              '(no restrictions so not doing it)
              '(no retrievals so not doing it)) )
      (let [ans (querydb (cons 'and restrictions)
                         (cons 'list (reverse retrievals)))]
        (if postprocess
            (eval (subst (if (cons? ans) ans '()) '$$ postprocess))
            ans )) ) )
