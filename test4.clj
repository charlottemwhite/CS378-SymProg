...
(defn findminmax [e v]
  (let [derivative (deriv (rhs e) v)
        solved-eq (solve (list '= 0 derivative) v)
        simplified (simplify (rhs solved-eq))]
    (list '= v simplified))
)
...
; add to the following
  ...
  ( (?function mintree) (?combine min) (?baseanswer (if (number? tree) tree 99999999)))
  ( (?function conses) (?combine add) (?baseanswer 0))
  ))

  (defn solve-helper [algpats e v]
  (if (empty? algpats)
    nil
    (if (solve (transform (first algpats) e) v)
      (transform (first algpats) e)
      (solve-helper (rest algpats) e v)))
)
         
(defn solve [e v]
  (cond
    (= (lhs e) v) e
    (= (rhs e) v) (list (op e) v (lhs e))
    (cons? (rhs e)) (solve (solve-helper algpats e v) v)
    :else nil)
)
...
(def javapats
  '(   ( (if ?test ?then) (if " (" ?test ")" \return \tab  ?then))
       ( (< ?x ?y)  ("(" ?x " " < " " ?y ")"))
       ...
       ( (if ?test ?then ?else) ("" "if" " " "(" ?test ") " "{" \tab \return
               ?then zuntab \return "}" \return "else " ?else \tab ))
       ( (return ?x) ("" "return" " " ?x ";"))
