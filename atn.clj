; atn.clj           Gordon S. Novak Jr.      ; 06 May 20
; 29 Apr 19; 06 Dec 19; 09 Dec 19
...
; using some, perhaps less likely to get stack overflow
(defn findwd [word lst]
  (some (fn [x] (or (and (= word x) word)
                    (and (cons? x) (= word (first x)) x) ) )
        lst))
...
; define a lexicon item, (category (words))
(defn deflexitem [item]
  (if (assocl (first item) lexicon)
      (def lexicon
           (map (fn [x] (if (= (first x) (first item)) item x)) lexicon))
      (def lexicon (cons item lexicon)) ) )

; define a group of lexicon items
(defn deflexicon [lst]
  (doseq [item (reverse lst)] (deflexitem item)) )
