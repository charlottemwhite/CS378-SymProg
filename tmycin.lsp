; tmycin.lsp         Common Lisp version           ; 29 Nov 06; 13 Nov 24
;
; TMYCIN = Tiny EMYCIN-like Expert System Tool
;
; Copyright (c) 2024  Gordon S. Novak Jr. and The University of Texas at Austin
;
...
lp  (terpri)
    ...
    (return (if (consp inp)
	        (if (and (symbolp (car inp))
                         (numberp (cadr inp)))
                    (list inp)
                    (mapcar #'fixinput inp))
		(list (list inp 1.0)) )) ))

; fix various forms of input in list
(defun fixinput (x)
  (if (symbolp x)
      (list x 1.0)
      (if (consp x)
